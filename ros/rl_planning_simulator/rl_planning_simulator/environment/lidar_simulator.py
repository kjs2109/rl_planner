...
class LidarSimlator():
    def __init__(self, lidar_range=10.0, lidar_num:int = 120):
        ...
        for a in range(lidar_num):
            self.lidar_lines.append(LineString(((0,0), (math.cos(a*math.pi/lidar_num*2)*lidar_range, math.sin(a*math.pi/lidar_num*2)*lidar_range))))
    ...
    def _fast_calc_lidar_obs(self, obstacles:list):
        ...
        for obst in obstacles:
            ...
            if hasattr(obst, 'exterior'):
                obst_coords = np.array(obst.exterior.coords)
            else:
